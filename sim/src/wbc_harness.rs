...
///
/// `namiashi_meas.misa` is the 現物合わせ result (2026-09-13): 3.295 kg
/// measured total, 611 g measured per leg, and -- the part the 3p3 variants
/// had to guess -- the MEASURED motor placement (MG4005 150 g each: the
/// hip-roll motor on the trunk, the thigh motor on the hip link, the knee
/// motor at the thigh's proximal end). It also carries the knee's 9:14
/// reduction referred through and the calibrated calf range (+-2.697 rad).
/// Generated by `remass_meas.py` in `models/namiashi/` (the
/// namiashi_description submodule); the copy here keeps this crate
/// standalone.
///
/// The `3p3_{prop,hip}` variants remain as the two bracketing hypotheses
/// from before the placement was known. The recorded prop-vs-hip
/// comparisons in `wbc_walk.rs` pin them explicitly and stay meaningful;
/// the measured distribution sits between them (motor mass proximal like
/// `hip`, but a motor on the thigh too, which neither variant has).
pub const DEFAULT_MISA: &str = "namiashi_meas.misa";
